...
use app\models\db\{Amendment, Consultation, IMotion, AmendmentSupporter, Motion, MotionSupporter, User};
...
$myMotionsReplace = \app\models\layoutHooks\Layout::getConsultationMyMotionsReplacer();
if ($myMotionsReplace !== null) {
    echo $myMotionsReplace;
    return;
}

$myMotions = $myself->getMySupportedMotionsByConsultation($consultation);
$myAmendments = $myself->getMySupportedAmendmentsByConsultation($consultation);

$getMyMotionAttrs = function (IMotion $motion, \app\models\db\ISupporter $supporter) {
    $class = [];
    $title = '';
...
if (count($myMotions)) {
    ...
    foreach ($myMotions as $motionSupport) {
        ...
        if ($motion->status === IMotion::STATUS_DRAFT) {
...
if (count($myAmendments) > 0) {
    ...
    foreach ($myAmendments as $amendmentSupport) {
        ...
        echo '<li class="' . $class . '"><div class="firstLine">';
        if ($amendment->status === IMotion::STATUS_DRAFT) {
