...
test.describe('Merging: deleted referenced by modification', () => {
    ...
    test('merge view handles deleted amendment with proposed procedure', async ({ page }) => {
        ...
        await test.step('set an amendment with a proposed procedure to invisible', async () => {
            await page.locator('#amendmentStatus').first().selectOption('-3');
            ...
            await expect(page.locator('.amendment281').first()).not.toBeVisible();
            await page.evaluate(() => {
            ...
            await expect(page.locator('body')).not.toContainText('Ä3', { useInnerText: true });
