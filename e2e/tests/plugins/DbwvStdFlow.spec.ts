...
async function loginAsDbwvTestUser(page: Page, username: string): Promise<void> {
    ...
    await page.locator('#username').first().fill(`${username}@example.org`);
    await page.locator('#passwordInput').first().fill('Test');
    await page
...
test.describe('DbwvStdFlow', () => {
    ...
    test('full DBWV motion flow through all roles', async ({ page, request }) => {
        ...
        await test.step('Antragsberechtigte: Antrag anlegen', async () => {
            ...
            await expect(page.locator('.myMotionList').filter({ visible: true })).toHaveCount(0);
            await page.locator('.btnCreateMotion').click();

            await page.locator("[name='sections[1]']").first().fill('Testantrag');
            ...
            await page.locator('#resolutionDate').first().fill('23.05.2020');
        ...
        await test.step('AL Recht: Sachgebiet zuordnen', async () => {
        ...
        await expect(page.locator('.adminMotionListActions').filter({ visible: true })).toHaveCount(0);
        await expect(page.locator('.actionCol').filter({ visible: true })).toHaveCount(0);
        await page.locator('.motion1 .prefixCol a').click();
        await expect(page.locator('#dbwv_main_tagSelect').first()).toBeVisible();
        await page.evaluate(() => {
        ...
        await expect(page.locator('#adminTodo').getByText('To Do (1)').filter({ visible: true })).toHaveCount(0);
        await logout(page);

        await loginAsDbwvTestUser(page, 'lv-sued-referat-iii');
        await expect(page.locator('#adminTodo')).toContainText('To Do (1)');
        await expect(page.locator('#dbwv_step1_subtagSelect').first()).toBeVisible();
        await expect(page.locator('#dbwv_step1_subtagNew').filter({ visible: true })).toHaveCount(0);
        await page.evaluate(() => {
        ...
        await expect(page.locator('#dbwv_step1_subtagNew').first()).toBeVisible();
        await page.locator('#dbwv_step1_subtagNew').first().fill('Gehalt');
        await expect(page.locator('#dbwv_step1_prefix')).toHaveValue('III/01');
        await page.locator('#dbwv_step1_textchanges').first().check();
        ...
        await expect(page.locator('#dbwv_step1_assign_number').first()).toBeVisible();
        await expect(page.locator('#dbwv_assign_main_tag').first()).toBeVisible();
        ...
        await page.goto('/std/lv-sued');
        await loginAsDbwvTestUser(page, 'lv-sued-antragsberechtigt-0');
        await test.step('Antragsberechtigte: Nur V1 sichtbar', async () => {
            ...
            await expect(page.locator('body')).not.toContainText('Third paragraph', { useInnerText: true });
            await expect(page.locator('.motionHistory').filter({ visible: true })).toHaveCount(0);
            await logout(page);
        ...
        await test.step('Büroleitung: Antrag freischalten', async () => {
        ...
        await test.step('Delegierte: Antrag sehen', async () => {
        ...
        await test.step('Andere Antragstellende: Antrag nicht sichtbar', async () => {
        ...
        await test.step('Antragstellende des Antrags: können Antrag inkl. Historie sehen', async () => {
            await expect(page.locator('body')).toContainText('Testantrag');
        ...
        await test.step('LV-Vorstand: kann Antrag inkl. Historie sehen', async () => {
            await expect(page.locator('.tagLink47').filter({ visible: true })).toHaveCount(0);
            await expect(page.locator('body')).not.toContainText('Testantrag', { useInnerText: true });
            await expect(page.locator('body')).not.toContainText(
                'Sachgebiet III - Dienst- und Laufbahnrecht',
                { useInnerText: true },
        ...
        await test.step('Referat: kann nicht mehr bearbeiten', async () => {
            await expect(page.locator('.tagLink47').filter({ visible: true })).toHaveCount(0);
            await expect(page.locator('body')).not.toContainText(
                'Sachgebiet III - Dienst- und Laufbahnrecht',
                { useInnerText: true },
            ...
            await page.locator('.tagLink47').click();
            await page.locator('.motionLink2').click();
            await expect(page.locator('#dbwv_step1_assign_number').filter({ visible: true })).toHaveCount(0);
            await logout(page);
        ...
        await expect(page.locator('#pp_section_2').getByText('Lorem ipsum dolor').filter({ visible: true })).toHaveCount(0);
        ...
        await test.step('Koordinierungsausschuss: Übernahme in HV', async () => {
            await expect(page.locator('#adminTodo').getByText('To Do (1)').filter({ visible: true })).toHaveCount(0);
        ...
        await expect(page.locator('#adminTodo').getByText('To Do (1)').filter({ visible: true })).toHaveCount(0);
        await expect(page.locator('#pp_section_2').filter({ visible: true })).toHaveCount(0);
        await expect(page.locator('#section_2_2')).toContainText('Dritter Absatz');
        await expect(page.locator('#section_2_2').getByText('Third paragraph').filter({ visible: true })).toHaveCount(0);

        await page.goto('/std/lv-sued');
        await page.locator('.tagLink47').click();
        await expect(page.locator('.resolutionList .motionLink4').filter({ visible: true })).toHaveCount(0);
        ...
        await clickJS(page, '.motionHistory .historyOpener button');
        await expect(page.locator('.motionHistory .otherVersion').getByText('V1').filter({ visible: true })).toHaveCount(0);
        ...
        await page.locator('#proposedChangeTextForm [name="save"]').click();
        await expect(page.locator('#pp_section_5').getByText('Lorem ipsum dolor').filter({ visible: true })).toHaveCount(0);
        ...
        await test.step('Redaktionsausschuss: Beschluss erarbeiten', async () => {
            await expect(page.locator('#adminTodo')).toContainText('To Do (1)');
            await page.locator('#adminTodo').click();
        ...
        await test.step('Beschlussveröffentlichung', async () => {
            await expect(page.locator('#adminTodo').getByText('To Do (1)').filter({ visible: true })).toHaveCount(0);
            ...
            await expect(page.locator('#adminTodo').getByText('To Do (1)').filter({ visible: true })).toHaveCount(0);
            await expect(page.locator('#pp_section_5').filter({ visible: true })).toHaveCount(0);
            await expect(page.locator('#section_5_2')).toContainText('Vierter Absatz');
            await expect(page.locator('#section_5_2').getByText('Dritter Absatz').filter({ visible: true })).toHaveCount(0);
            ...
            await expect(page.locator('#adminTodo').getByText('To Do (1)').filter({ visible: true })).toHaveCount(0);
            await expect(page.locator('body')).not.toContainText('Begründung', { useInnerText: true });
