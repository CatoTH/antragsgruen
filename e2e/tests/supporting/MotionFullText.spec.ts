...
test.describe('Supporting: MotionFullText', () => {
    ...
    test('full text supporter adder visible for admins only and works correctly', async ({ page }) => {
        ...
        await test.step('check that the field is not visible for normal users', async () => {
        ...
        await test.step('check that the field is visible for admins', async () => {
            await page.locator('.createMotion').click();
            await expect(page.locator('.supporterData').first()).toBeVisible();
        ...
        await test.step('check that the function actually works', async () => {
            await page.locator('#supporterFullTextHolder textarea').first().fill('Tobias Hößl, KV München; Test 2');
