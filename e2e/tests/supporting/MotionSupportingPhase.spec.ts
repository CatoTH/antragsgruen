...
test.describe('Supporting: MotionSupportingPhase', () => {
    ...
    test('motion supporting phase flow', async ({ page }) => {
        ...
        await test.step('check the admin settings and enable gender support and the supporting page', async () => {
        ...
        await test.step('enable/disable liking and disliking', async () => {
            ...
            await expect(page.locator('section.likes form').filter({ visible: true })).toHaveCount(0);
            ...
            await loginAsStdUser(page);
            await page.goto(`/${motionUrl}`);
            await expect(page.locator('section.likes form').first()).toBeVisible();
        ...
        await test.step('revoke the support', async () => {
            ...
            await expect(page.locator('.bootbox').first()).toBeVisible();
        ...
        await test.step('submit the motion', async () => {
            await expect(page.locator('section.supporters').getByText('Testuser').filter({ visible: true })).toHaveCount(0);
            ...
            await page.locator('#motionSupportGender').first().selectOption('Keine Angabe');
            await page.locator('.motionSupportForm [name="motionSupport"]').click();
            await expect(page.locator('body')).not.toContainText('Du unterstützt diesen Antrag nun.', { useInnerText: true });
            ...
            await page.locator("input[name='motionInitiatorSettings[contactGender]'][value='0']").first().check();
        });

        await test.step('Disable gender support because somehow the test fails later on otherwise ;_;', async () => {
            await page.locator('.adminTypeForm [name="save"]').click();
        ...
        await test.step('check that motions created as normal person are in supporting phase', async () => {
            await page.locator('#sections_30').first().fill('Title as normal person');
        ...
        await test.step('check that motions created as organizations are not in supporting phase', async () => {
            await page.locator('#sections_30').first().fill('Title as organization');
            await setCkEditorContent(page, 'sections_31_wysiwyg', '<p><strong>Test</strong></p>');
            await page.locator('#personTypeOrga').first().check();
            await page.locator('#initiatorPrimaryName').first().fill('My organization');
