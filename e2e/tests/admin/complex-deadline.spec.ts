...
test.describe('Admin: ComplexDeadline', () => {
    ...
    test('complex deadline: 6 phases covering motions/amendments/comments/merging', async ({ page }) => {
        ...
        await test.step('set up the deadline test case', async () => {
            await expect(page.locator('.stickyAdminDebugFooter').filter({ visible: true })).toHaveCount(0);
            await expect(page.locator('.deadlineTypeComplex.motionDeadlines').filter({ visible: true })).toHaveCount(0);
            await expect(page.locator('#typeDeadlineMotionsHolder').first()).toBeVisible();
            await page.locator('#deadlineFormTypeComplex').first().check();
            await expect(page.locator('.deadlineTypeComplex.motionDeadlines').first()).toBeVisible();
            await expect(page.locator('#typeDeadlineMotionsHolder').filter({ visible: true })).toHaveCount(0);
            ...
            await page.locator('#deadlineDebugMode').first().check();
            await page.locator('.adminTypeForm [name="save"]').first().click();
            await expect(page.locator('.stickyAdminDebugFooter').first()).toBeVisible();

            async function setTime(date: string) {
                await new ConsultationHomePage(page).open();
                await page.locator('#simulateAdminTimeInput').first().fill(date);
            ...
            await expect(page.locator('.commentForm').first()).toBeVisible();
            await expect(page.locator('.amendmentCreate .onlyAdmins').first()).toBeVisible();
            ...
            await new MotionPage(page).open({ motionSlug: '321-o-zapft-is' });
            await expect(page.locator('.commentForm').first()).toBeVisible();
            await expect(page.locator('.amendmentCreate .onlyAdmins').filter({ visible: true })).toHaveCount(0);
            await expect(page.locator('.amendmentCreate').first()).toBeVisible();
            await expect(page.locator('#sidebar .mergeamendments').filter({ visible: true })).toHaveCount(0);
            ...
            await expect(page.locator('.amendmentCreate .onlyAdmins').first()).toBeVisible();
            await expect(page.locator('#sidebar .mergeamendments').first()).toBeVisible();
        });
    });
});