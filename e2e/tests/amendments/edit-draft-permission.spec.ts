...
test.describe('Amendments: EditDraftPermission', () => {
    ...
    test('create and edit a draft logged out', async ({ page }) => {
        ...
        await test.step('create a draft amendment logged out', async () => {
    ...
    test('create and edit a draft logged in', async ({ page }) => {
        ...
        await page.locator('#initiatorEmail').first().fill('test@example.org');
        await page.locator('#amendmentEditForm [name="save"]').click();
        await test.step('create a draft amendment logged in', async () => {
            await expect(page.locator('h1')).toContainText(/Änderungsantrag bestätigen/i);
    ...
    test('edit the draft logged out (should not work)', async ({ page }) => {
        ...
        await expect(page.locator('h1').getByText('ÄNDERUNGSANTRAG ZU A3: TEXTFORMATIERUNGEN STELLEN').filter({ visible: true })).toHaveCount(0);
        await expect(page.locator("input[name='sections[1]']").filter({ visible: true })).toHaveCount(0);
    });
});