...
test.describe('Appearance: edit content pages', () => {
    ...
    test('create, edit, restrict, unpublish, delete a content page', async ({ page }) => {
        ...
        await expect(page.locator('#mainmenu .addPage')).toHaveCount(0);

        await loginAsConsultationAdmin(page);
        await expect(page.locator('#mainmenu .addPage')).toBeVisible();

        await page.locator('#mainmenu .addPage a').click();

        await page.locator('.createPageForm #contentUrl').fill('about');
        await page.locator('.createPageForm #contentTitle').fill('About');
        ...
        await expect(page.locator('.contentSettingsToolbar')).toBeVisible();

        await setCkEditorContent(page, 'stdTextHolder', '<p>New text</p>');

        await page.locator('.contentSettingsToolbar #contentUrl').fill('images');
        await page.locator('.contentSettingsToolbar #contentTitle').fill('Images');

        await expect(page.locator("input[name='inMenu']")).toBeChecked();
        await expect(page.locator('.userGroupSelect')).toHaveCount(0);

        await page.locator('#policyReadPage').selectOption('3');
        await page.waitForTimeout(200);
        await expect(page.locator('.userGroupSelect')).toBeVisible();
        ...
        await expect(page.locator('.contentSettingsToolbar')).toHaveCount(0);
        ...
        await page.locator("input[name='allConsultations']").check();
        await page.locator("input[name='inMenu']").uncheck();
        await page.locator('.submitBtn').click();
        await page.waitForTimeout(1000);
        ...
        await expect(page.locator('.createPageForm')).toBeVisible();
        await expect(page.locator('body')).not.toContainText('Images');
