...
test.describe('Appearance: resolutions displayed separately', () => {
    ...
    test('resolutions separate from motions across all start layouts', async ({ page }) => {
        ...
        await test.step('create a resolution', async () => {
        ...
        await test.step('See the resolution on the default home page', async () => {
            await expect(
                page.locator(
            ...
            await expect(page.locator('.sectionResolutions').getByText('A2').filter({ visible: true })).toHaveCount(0);
            ...
            for (const [layoutId, _name] of layouts) {
                ...
                await expect(page.locator('.showResolutionsSeparateHolder').first()).toBeVisible();
                await page.evaluate(() => {
                    const el = document.querySelector(
                ...
                await expect(page.locator('.motionLink2').filter({ visible: true })).toHaveCount(0);
                await expect(
                    page.locator(`.motionLink${FIRST_FREE_MOTION_ID + 1}`),
                ).toBeVisible();
                ...
                if (layoutId === '3') {
                    ...
                    await expect(page.locator('.motionList')).toContainText('O’zapft is!');
                    await expect(page.locator('.motionList').getByText('A2').filter({ visible: true })).toHaveCount(0);
                }
