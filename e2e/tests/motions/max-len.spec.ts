...
test.describe('Maximum section length', () => {
    ...
    test('max length settings are saved', async ({ page }) => {
        ...
        await test.step('activate line length', async () => {
            await expect(page.locator('.section1 .maxLenInput').filter({ visible: true })).toHaveCount(0);
            await page.locator('.section1 .maxLenSet').first().check();
            await expect(page.locator('.section1 .maxLenInput').first()).toBeVisible();
            await page.locator('.section1 .maxLenInput input').first().fill('20');
            await page.locator('.section2 .maxLenSet').first().check();
            await page.locator('.section2 .maxLenInput input').first().fill('100');
            await page.locator('.section3 .maxLenSet').first().check();
            await page.locator('.section3 .maxLenInput input').first().fill('150');
            await page.locator('.section3 .maxLenSoftCheckbox').first().check();
    ...
    test('too long texts block submitting unless the limit is soft', async ({ page }) => {
        ...
        await test.step('create a motion', async () => {
            await expect(page.locator('body')).not.toContainText('Der Text ist zu lang', { useInnerText: true });
            ...
            await page.locator('#sections_1').first().fill('12345');
            await expect(page.locator('body')).not.toContainText('Der Text ist zu lang', { useInnerText: true });
            await expect(page.locator('body')).toContainText('Max. 20 Zeichen (Aktuell: 5)');

            await page.locator('#sections_1').first().fill('x'.repeat(21));
            ...
            await page.locator('#sections_1').first().fill('x'.repeat(20));
