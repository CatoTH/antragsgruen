...
test.describe('Initiator confirmation emails', () => {
    ...
    test('confirmation mails are only sent when the initiator submits themselves', async ({
        ...
        await page.locator('#initiatorConfirmEmails').first().check();
        ...
        await expect(page.locator('body')).not.toContainText('E-Mail sent to: test@example.org', { useInnerText: true });
    });
    ...
    test('amendment confirmation mails follow the same rule', async ({ page }) => {
        ...
        await page.locator("input[name='otherInitiator']").first().uncheck();
        await test.step('create an amendment as user', async () => {
        ...
        await test.step('create an amendment as admin', async () => {
            await page.locator('#sections_1').first().fill('New title');
            await page.locator('#initiatorPrimaryName').first().fill('My Name');
            await page.locator('#initiatorEmail').first().fill('test@example.org');
            await page.locator('#amendmentEditForm [name="save"]').click();
    ...
    test('screening sends the confirmation mail to the initiator', async ({ page }) => {
        ...
        await test.step('screen the motion / amendment', async () => {
