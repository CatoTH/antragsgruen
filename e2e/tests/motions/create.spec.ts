...
test.describe('Motion creation (basic flow)', () => {
    ...
    test('motion Create site loads', async ({ page }) => {
        await test.step('motion Create site loads', async () => {
            ...
            await expect(page.locator('body')).not.toContainText('Beschlussdatum', { useInnerText: true });
            await expect(page.locator('body')).not.toContainText('Ansprechperson', { useInnerText: true });
            ...
            await expect(page.locator('#section_holder_3 label.optional').first()).toBeVisible();
            await expect(page.locator('#section_holder_2 label.required').first()).toBeVisible();
        });

        await test.step('submit without resolution date shows bootbox error', async () => {
            const home = new (await import('../../pages/ConsultationHomePage')).ConsultationHomePage(page);
            await home.gotoMotionCreatePage();

            await test.step('create a regular motion, but forgot the organization and resolution date', async () => {
                await page.locator("input[name='tags[]'][value='1']").first().check();
            });

            await test.step('finally create the motion for real', async () => {
                ...
                await page.locator("[name='Initiator[primaryName]']").first().fill('Mein Name');
                await page.locator("[name='Initiator[contactEmail]']").first().fill('test@example.org');
                await page.locator("[name='Initiator[contactPhone]']").first().fill('+49123456789');
        ...
        await test.step('create, correct, re-submit, and confirm motion end-to-end', async () => {
            const home = new (await import('../../pages/ConsultationHomePage')).ConsultationHomePage(page);
            await home.gotoMotionCreatePage();
            ...
            await page.locator("[name='Initiator[contactPhone]']").first().fill('+49123456789');
            await page.locator('#personTypeOrga').first().check();
            await page.locator("[name='Initiator[resolutionDate]']").first().fill('12.01.2015');
            await page.locator("[name='Initiator[contactName]']").first().fill('MeinKontakt');
            await page.locator('#motionEditForm [name="save"]').click();

            await expect(page.locator('h1')).toContainText(/antrag bestätigen/i);

            await test.step('not confirm the motion, instead correcting a mistake', async () => {
                await page.locator('#motionConfirmForm [name="modify"]').click();
                await expect(page.locator('h1')).toContainText(/antrag stellen/i);
            });

            await test.step('make some changes to the motion', async () => {
                await page.locator("[name='sections[1]']").first().fill('Testantrag 2');
            ...
            await test.step('finally submit the motion', async () => {
                await page.locator("[name='Initiator[primaryName]']").first().fill('');
                await page.locator("[name='Initiator[contactEmail]']").first().fill('test2@example.org');
                await page.locator("[name='Initiator[contactPhone]']").first().fill('+49-123-456789');
                await page.locator('#motionEditForm [name="save"]').click();
                ...
                await page.locator("[name='Initiator[primaryName]']").first().fill('My real name');
                await page.locator('#motionEditForm [name="save"]').click();
            ...
            await test.step('confirm the submitted motion', async () => {
            ...
            await test.step('check the visible data', async () => {
                ...
                await expect(page.locator('body')).toContainText('My real name');
                await expect(page.locator('body')).not.toContainText('test2@example.org', { useInnerText: true });
                await expect(page.locator('body')).not.toContainText('+49-123-456789', { useInnerText: true });
            });
        });

        await test.step('contact details visible only to admins', async () => {
