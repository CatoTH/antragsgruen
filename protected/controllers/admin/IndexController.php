...
class IndexController extends AntragsgruenController
{
    ...
    public function actionAdminsReihe($veranstaltungsreihe_id = "", $veranstaltung_id = "")
    {
        $this->loadVeranstaltung($veranstaltungsreihe_id, $veranstaltung_id);
        if (!$this->veranstaltung->isAdminCurUser()) $this->redirect($this->createUrl("/veranstaltung/login", array("back" => yii::app()->getRequest()->requestUri)));
        ...
        if (AntiXSS::isTokenSet("adduser")) {
            ...
            if ($user) {
                ...
                Yii::app()->user->setFlash("error", "Benutzer \"" . $_REQUEST["username"] . "\" nicht gefunden. Hat die/derjenige sich schon einmal mit diesem Wurzelwerknamen bei Antragsgrün angemeldet?");
            }
        }
        ...
        $this->render('admins_reihe', array(
            "admins" => $this->veranstaltungsreihe->admins,
            "sprache" => $this->veranstaltung->getSprache(),
            "ich" => $ich,
            "del_url" => $this->createUrl("/admin/index/adminsReihe", array(AntiXSS::createToken("remove") => "REMOVEID")),
            "add_url" => $this->createUrl("/admin/index/adminsReihe")
        ));
