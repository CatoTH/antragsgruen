...
$submitUrl = UrlHelper::createUrl(['/dbwv/admin-workflow/step6decide', 'motionSlug' => $motion->getMotionSlug()]);

if ($motion->version === \app\plugins\dbwv\workflow\Workflow::STEP_V7) {
...
echo Html::beginForm($submitUrl, 'POST', [
    'id' => 'dbwv_step6_decide',
    'class' => 'dbwv_step dbwv_step6_decide',
]);
?>
        ...
        <div style="display: flex; width: 100%; flex-direction: row;">
            ...
            <div style="flex-basis: 50%;">
                ...
                <div style="padding-left: 16px;">
                    ...
                    <label id="dbwv_step6_custom_str" class="hidden">
...
?>
    <div style="padding: 10px;">
        ...
        <div class="form-group wysiwyg-textarea single-paragraph">
            <label for="dbwv_step6_protocol" class="hidden"><?= Yii::t('motion', 'protocol') ?>:</label>
            <textarea id="dbwv_step6_protocol" name="protocol"></textarea>
            <div class="texteditor boxed motionTextFormattings" id="dbwv_step6_protocol_wysiwyg"><?php
    ...
    <div style="text-align: right; padding: 10px;">
        <button type="submit" class="btn btn-primary">
            ...
            V7 Beschluss festlegen
        </button>
    </div>

    <script>
        $(function() {
            $("#dbwv_step6_decide input[name=followproposal]").on("change", function() {
                if ($("#dbwv_step6_decide input[name=followproposal]:checked").val() === "no") {
                    $("#dbwv_step6_decide input[name=decision]").prop("disabled", false).prop("required", true);
                } else {
                    $("#dbwv_step6_decide input[name=decision]").prop("disabled", true).prop("required", false);
                }
            }).trigger("change");

            $("#dbwv_step6_decide input[name=decision]").on("change", function() {
                if ($("#dbwv_step6_decide input[name=decision]:checked").val() == <?= IMotion::STATUS_CUSTOM_STRING ?>) {
                    $("#dbwv_step6_custom_str").removeClass('hidden');
                } else {
                    $("#dbwv_step6_custom_str").addClass('hidden');
                }
            }).trigger("change");
            ...
            let $el = $("#dbwv_step6_protocol_wysiwyg");
            $el.data("ckeditor_initialized", "1");
            $el.attr("contenteditable", "true");

            CKEDITOR.inline('dbwv_step6_protocol_wysiwyg', ckeditorConfig);

            $("#dbwv_step6_decide").on("submit", function() {
                let data = CKEDITOR.instances['dbwv_step6_protocol_wysiwyg'].getData();
                $("#dbwv_step6_protocol").val(CKEDITOR.instances['dbwv_step6_protocol_wysiwyg'].getData());
            });
        });
    </script>
<?php
