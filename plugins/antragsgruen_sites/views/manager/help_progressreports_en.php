...
<div class="content managerHelpPage">
    ...
    <ul>
        ...
        <li>The progress report has a „last changed on“ and „author“ field that is independent of the resolution itself.</li>
    </ul>
    ...
    <p>While creating the resolution (as part of the „Merge amendments“ function), you will be presented a choice if to create a new motion, a preliminary resolution or a final resolution. If you choose a resolution, and a compatible motion type exists (see „Setup“), a dropdown with compatible motion types will appear, the progress reports type being one of them. Select it and proceed as normal.</p>
    ...
    <p>If you use the default motion type for motions (that is, a title, a motion text, and optionally a reason), it will be fairly simple to create a motion type for the progress reports: In the settings, choose „Create new motion type“, and select „Progress reports“ as template there. A new motion type will be created with the following noteworthy settings:</p>
    <ul>
        <li>Only Admins are allowed to create new documents (resolutions)</li>
        <li>Besides the default sections „Title“ and „Resolution“, there will not be a „Reason“ (as the reason is typically removed when creating a resolution) section, but a „Progress“ section of type „Editorial Text“ instead.</li>
    </ul>
    ...
    <p>The relevant permission to edit progress reports is called „Manage editorial sections / Progress reports“. Users can receive this permission in three ways:</p>
    <ul>
        <li>Consultation and Site admins have this permission automatically</li>
        <li>You can assign users to the user group „Progress reports“. This group grants said permission to all members.</li>
        <li>Alternatively, you can create a new user group, add members, and grant this permission to this group. This would be the approach to take if you want to define users that can only edit the progress reports of a subset of all resolutions - for example, having different user groups, with each being responsible for a specific Topic / Tag. (Use the „Restricted to: Tag“ choice when defining the permissions in this case).</li>
    </ul>
</div>
