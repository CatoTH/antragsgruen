...
<div class="content managerHelpPage">
    ...
    <p>We <a href="/help/member-motion">already explained</a> how to allow users to submit their own motions. If the documents are provided by admins instead (that is, the permission to submit a motion is set to „Nobody“ or „Admins“), you will find the link to create the base document in the admin motion list, at the top of the page a prominent „Create“ button. There, you will have the choice which type of motion to create, independent of any permissions.</p>
    ...
    <p>A hint regarding custom document templates: if you create a custom template with different kinds of section types (like PDF attachments or image uploads), please mind that only sections of the simple type „Text“ and „Title“ are amendable, and only if they are marked as such via the checkbox „In Amendments“. There is no issue with having multiple text sections being amendable, if wanted, though. The default template for motions has the „Reason“ section set to be non-amendable, as this is how most organizations have their procedures set up regards to amendments, but this can be set up differently by simply activating the „In Amendments“ checkbox.</p>

    <h3 id="permissions">Permissions</h3>

    <p>Setting up who is allowed to create amendments for a motion of a particular motion type is very similar to setting up who is allowed to create the motion itself. That is, at the motion type under „Permissions“ -> „Create amendments“. A more detailed explanation of the different options can be found in the tutorial on how to set up motions.</p>

    <h3 id="singleparagraph">Restricting amendments to one paragraph</h3>

    <p>Next to the permissions, there is an additional option for amendments: „Amendments may only change one paragraph“. As the name already indicates, it controls if members can create amendments that are changing the text all over the document, or if they need to restrict themselves to one paragraph that has to be chosen by the user before modifying it. Which version to prefer depends a lot on the dynamics of the respective organization and the kinds of changes that are to be encouraged by the system:</p>
    <ul>
    ...
    <p>If the amendment is restricted to one paragraph, this can be restricted even further by selecting „…and only one specific place“. This can be chosen if only single words or phrases are to be changed. As this is very restrictive, we recommend to not use this unless amendments are really only to change individual words, numbers etc.</p>
    ...
    <p>Both options can be enabled and disabled in the consultation settings (not at the motion type, for a change). More specifically, at „Settings“ -> „This Consultation“ -> „Amendments“, the two points:</p>
    <ul>
