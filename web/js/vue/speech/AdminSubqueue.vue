<template>
  <section class="subqueue" v-t:aria-label="['speech', 'waiting_list', false, {}, ' ' +subqueue.name]"
           :class="{ positionLeft: (position === 'left'), positionRight: (position === 'right') }"
  >
    <header v-if="subqueue.name !== 'default'">{{ subqueue.name }}</header>
    <header v-if="subqueue.name === 'default'" v-t="['speech', 'waiting_list']"></header>
    ...
    <ul class="subqueueItems">
      <template v-for="(item, index) in subqueue.applied">
        <li class="dropPlaceholder" :class="{hovered: (index === hoveredPlaceholder), hoverable: isHoverable(index)}">
          ...
          <div class="hoveredIndicator" v-t="['speech', 'admin_move_here']"></div>
        </li>
    ...
    <div class="empty" v-if="subqueue.applied.length === 0" v-t="['speech', 'admin_subq_no_applic']"></div>
    ...
    <section class="subqueueAdder">
      <button class="link adderOpener" type="button" v-if="!adderOpened" v-on:click="openAdder()" v-on:keyup.enter="openAdder()">
        ...
        <template v-t="['speech', 'admin_subq_add']"></template>
      </button>
      <form method="POST" v-on:submit="onAdderSubmitted($event)" v-if="adderOpened">
        <label v-bind:for="'subqueueAdderName' + subqueue.id" class="sr-only" v-t="['speech', 'admin_subq_name']"></label>
        <div class="input-group">
          <input type="text" class="form-control" ref="adderNameInput" v-model="adderName" v-bind:id="'subqueueAdderName' + subqueue.id"
                 required v-t:placeholder="['speech', 'admin_subq_name']" v-t:title="['speech', 'admin_subq_name']">
          <span class="input-group-btn">
                    <button class="btn btn-default" type="submit" v-t="['speech', 'admin_subq_add']"></button>
                </span>
        </div>
      </form>
    </section>
