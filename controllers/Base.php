...
class Base extends Controller
{
    ...
    {
        $urlparams = array($route);
        $p = explode("/", $route);
        if ($p[0] != "manager") {
            if (!isset($params["consultation_id"]) && $this->consultation !== null) $params["consultation_id"] = $this->consultation->url_path;
            if ($this->getParams()->multisiteMode && !isset($params["site_id"]) && $this->site != null) $params["site_id"] = $this->site->subdomain;
            if ($route == "consultation/index" && !is_null($this->site) && strtolower($params["consultation_id"]) == strtolower($this->site->current_consultation->url_path)) unset($params["consultation_id"]);
