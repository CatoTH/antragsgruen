...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
  ...
  <input class="hidden hidden-field" name="' . $formName . '" readonly="readonly" ' .
     ...
     * @return string
     */
    public static function getMotionFormField($formId, $content, $title, $maxLen, $fullHtml, $fixedWidth, $forbidden)
    {
        $htmlId = 'sections_' . $formId;

        $str = '<div class="form-group wysiwyg-textarea" id="section_holder_' . $formId . '"';
        $str .= ' data-max-len="' . IntVal($maxLen) . '"';
        $str .= ' data-full-html="' . ($fullHtml ? '1' : '0') . '"';
        $str .= '><label for="sections_' . $formId . '">' . Html::encode($title) . '</label>';
        ...
        $str .= 'title="' . Html::encode($title) . '">';
        $str .= Html::encode($content) . '</textarea>';
        $str .= '<div class="texteditor motionTextFormattings boxed';
        if ($fixedWidth) {
            $str .= ' fixedWidthFont';
        }
        $str .= '" id="' . $htmlId . '_wysiwyg" ';
        if (in_array('strike', $forbidden)) {
