...
<section class="content" v-t:aria-label="['admin', 'siteacc_accounts_title']">
  <div class="filterHolder">
    ...
    <div class="usernameFilter">
      <div class="input-group">
        <input type="text" class="form-control" v-t:placeholder="['admin', 'siteacc_userfilter_place']"
               v-t:aria-label="['admin', 'siteacc_userfilter_aria']"
    ...
    <li v-for="user in usersFiltered" :class="'user' + user.id">
      ...
      <div class="groupsDisplay">
        ...
        <button type="button" class="btn btn-link btnEdit" @click="editUser(user)"
                v-t:title="['admin', 'siteacc_user_edit', false, {'%USERNAME%': user.name}]"
                v-t:aria-label="['admin', 'siteacc_user_edit', false, {'%USERNAME%': user.name}]">
          <span class="glyphicon glyphicon-wrench" aria-hidden="true"></span>
        </button>
        <button type="button" class="btn btn-link btnRemove" @click="removeUser(user)"
                v-t:title="['admin', 'siteacc_user_del', false, {'%USERNAME%': user.name}]"
                v-t:aria-label="['admin', 'siteacc_user_del', false, {'%USERNAME%': user.name}]">
...
<section aria-labelledby="userGroupsAdminTitle">
  <h2 class="green" id="userGroupsAdminTitle" v-t="['admin', 'siteacc_groups_title']"></h2>
  <div class="content">
    <ul class="groupList">
      <li v-for="group in sortedGroups" :class="'group' + group.id">
        ...
        <div class="groupActions">
          <button class="btn btn-link btnEdit" @click="editGroup(group)"
                  v-t:title="['admin', 'siteacc_group_edit', false, {'%GROUPNAME%': group.title}]"
                  v-t:aria-label="['admin', 'siteacc_group_edit', false, {'%GROUPNAME%': group.title}]">
            <span class="glyphicon glyphicon-wrench" aria-hidden="true"></span>
          </button>

          <button v-if="group.editable" class="btn btn-link btnRemove" @click="removeGroup(group)"
                  v-t:title="['admin', 'siteacc_group_del', false, {'%GROUPNAME%': group.title}]"
                  v-t:aria-label="['admin', 'siteacc_group_del', false, {'%GROUPNAME%': group.title}]">
            <span class="glyphicon glyphicon-trash" aria-hidden="true"></span>
          </button>
        </div>
      </li>
...
export default {
  ...
  computed: {
    userGroupFilter: function () {
      ...
      return [
        {
          label: translate.getTranslation('admin', 'siteacc_userfilter_allgr'),
          id: '',
        },
  ...
  methods: {
    ...
    removeUser: function (user) {
      const widget = this,
          str = translate.getTranslation('admin', 'siteacc_userdel_confirm').replace(/%USERNAME%/, user.name);
    ...
    removeGroup: function (group) {
      const widget = this,
          str = translate.getTranslation('admin', 'siteacc_groupdel_confirm').replace(/%GROUPNAME%/, group.title);
      bootbox.confirm(str, function(result) {
        if (result) {
