      ...
      <article class="modal-content">
        ...
        <main class="modal-body" v-if="user">
          <div class="stdTwoCols">
            <div class="leftColumn" v-t="['admin', 'siteacc_usermodal_identity']"></div>
            ...
            <div class="rightColumn" v-if="canModifyAuth">
              <div v-if="!settingAuth">
                ...
                <button class="btn btn-link" @click="openSetAuth()">
                  ...
                  <span class="sr-only" v-t="['admin', 'siteacc_usermodal_idnew']"></span>
                </button>
              </div>
              <div v-if="settingAuth">
                <input type="email" class="form-control" autocomplete="off" v-t:title="['admin', 'siteacc_usermodal_idnew']"
                       v-model="newAuth" ref="auth-setter">
              </div>
            </div>
          </div>
          <div class="stdTwoCols 2faRow">
            <div class="leftColumn" v-t="['admin', 'siteacc_usermodal_2fa']"></div>
          ...
          <div class="stdTwoCols" v-if="permissionGlobalEdit">
            <div class="leftColumn" v-t="['admin', 'siteacc_usermodal_pass']"></div>
            <div class="rightColumn">
              <button type="button" class="btn btn-sm btn-default btnSetPwdOpener"
                      v-if="!settingPassword" @click="openSetPassword()" v-t="['admin', 'siteacc_usermodal_passset']"></button>
              <input type="password" v-model="newPassword" class="form-control inputPassword"
                     v-if="settingPassword"
                     autocomplete="off"
                     v-t:title="['admin', 'siteacc_usermodal_passnew']"
                     v-t:placeholder="['admin', 'siteacc_usermodal_passnew']"
                     ref="password-setter"
              >
          ...
          <div class="stdTwoCols">
            <div class="leftColumn" v-t="['admin', 'siteacc_new_name_given']"></div>
          ...
          <div class="stdTwoCols">
            <div class="leftColumn" v-t="['admin', 'siteacc_new_name_family']"></div>
            <div class="rightColumn" v-if="!permissionGlobalEdit">
              {{ name_family }}
            </div>
            <div class="rightColumn" v-if="permissionGlobalEdit">
          ...
          <div class="stdTwoCols">
            <div class="leftColumn" v-t="['admin', 'siteacc_new_name_orga']"></div>
            <div class="rightColumn" v-if="!permissionGlobalEdit">
              {{ organization }}
            </div>
          ...
          <div class="stdTwoCols">
            <div class="leftColumn" v-t="['admin', 'siteacc_admins_vote_weight']"></div>
            <div class="rightColumn">
              <input type="text" class="form-control inputVoteWeight" v-model="voteweight">
            </div>
          </div>
          <div class="stdTwoCols">
            <div class="leftColumn" v-t="['admin', 'siteacc_admins_pp_replyto']"></div>
            <div class="rightColumn" v-if="!permissionGlobalEdit">
              {{ ppreplyto }}
            </div>
            <div class="rightColumn" v-if="permissionGlobalEdit">
          ...
          <small v-if="!permissionGlobalEdit" class="onlyGlobalAdminsHint" v-t="['admin', 'siteacc_usermodal_superh']"></small>
        </main>
        <footer class="modal-footer" v-if="!deletingVisible">
          ...
          <button type="submit" class="btn btn-primary btnSave" @click="save($event)" v-t="['base', 'save']"></button>
        </footer>
        <footer class="modal-footer" v-if="deletingVisible && permissionGlobalEdit">
          <button type="button" class="btn btn-danger btnDelete" @click="deleteAccount($event)" v-t="['admin','siteacc_useraccdel_btn']"></button>
        </footer>
      </article>
    </form>
  </div>
